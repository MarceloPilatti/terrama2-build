...
if [ ! -d "${PLUGINS_DIR}" ]; then
  ...
  if [ ! -f "${PLUGINS_DIR}/gt-imagepyramid*.jar" ]; then
    wget -qO geoserver-2.12.5-pyramid-plugin.zip -L "https://sourceforge.net/projects/geoserver/files/GeoServer/2.12.5/extensions/geoserver-2.12.5-pyramid-plugin.zip/download"
